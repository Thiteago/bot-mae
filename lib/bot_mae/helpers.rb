...
module DiscordBot
  class Helpers
    ...
    def has_been_stop_requested?
      DiscordBot::RedisCache.read("stop_playing_#{@details[:server_id]}_#{@details[:voice_channel_id]}")
    end
    ...
    def shift_queue
      user_queue = DiscordBot::RedisCache.read(@details[:user_queue])
      user_queue.shift
      DiscordBot::RedisCache.write(@details[:user_queue], user_queue)
    end

    def self.clean_queue(event)
      DiscordBot::RedisCache.write("#{event.server.id}_#{event.author.voice_channel.id}_server_queue", [])
    end
    ...
    def search_song
      ...
      if @details[:platform] == 'not_identified'
        video = DiscordBot::YoutubeSearchCrawler.search_video(@details[:requested_song])
        ...
        if @details[:type] == 'playlist'
          ...
          limit = 100
          offset = 0
          if DiscordBot::RedisCache.read(@details[:user_queue]).empty?
            video = DiscordBot::YoutubeSearchCrawler.search_video(playlist.tracks[0].name + " " + playlist.tracks[0].artists[0].name)
            enqueue_song(video[:video_id], video[:title]) if video
          ...
          while offset < playlist_length
            ...
            SearchVideoByPlaylistJob.perform_async(tracks, @details[:user_id], @details[:server_id], @details[:voice_channel_id])

            offset += current_limit
          end
        elsif @details[:type] == 'track'
          track = RSpotify::Track.find(@details[:requested_song].split('track/')[1])
          video = DiscordBot::YoutubeSearchCrawler.search_video(track.name + " " + track.artists.first.name)
          enqueue_song(video[:video_id],video[:title]) if video
        else
    ...
    def find_songs
      original_queue_size = DiscordBot::RedisCache.read(@details[:user_queue]).size
      if is_youtube_link?
        result = search_song
        if !result.nil?
          if DiscordBot::RedisCache.read(@details[:user_queue]).size > original_queue_size && original_queue_size > 0
    ...
    def enqueue_song(video_id, video_title)
      queue = DiscordBot::RedisCache.read(@details[:user_queue])
      queue << {id: video_id, title: video_title}
      DiscordBot::RedisCache.write(@details[:user_queue], queue)
    end
    ...
    def create_queue_embed(user_queue, requested_page=1)
      ...
      user_queue[start_index..end_index].each_with_index do |song, index|
        link = is_youtube_link?(song[:id]) ? song[:id] : "https://www.youtube.com/watch?v=#{song[:id]}"
