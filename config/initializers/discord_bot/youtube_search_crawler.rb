    ...
    def self.search_video_by_playlist(tracks, user_id, server_id)
      Puppeteer.launch(headless: true) do |browser|
        ...
        tracks.each do |track|
          ...
          if cached_result
            DiscordBot::Helpers.enqueue_song(cached_result[:video_id], cached_result[:title], user_id, server_id)
            next
          end
          ...
          DiscordBot::Helpers.enqueue_song(cached_data[:video_id], cached_data[:title], user_id, server_id)
        end
        page.close
      end
      []
